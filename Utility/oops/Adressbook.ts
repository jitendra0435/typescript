...
class Address {
    ...
    create() {
        ...
        let address:string = util.inputString("Input ");
    ...
    view() {
        ...
        let name:string = util.inputString(), viewed = false;
        for (let i = 0; i < object.list.length; i++) {
    ...
    update() {
        ...
        for (let i = 0; i < object.list.length; i++) {
            if (name == object.list[i].firstName) { var found=true;
                console.log("Choose the detail you want to update:");
                console.log("1.Address\n2.City\n3.State\n4.Zip Code\n5.Phone\n6.Main Menu");
                let choice:number = util.inputInt();
                switch (choice) {
                    case 1:
